...
/**
 * Servlet implementation class UusiVuokrausServlet
 */
@WebServlet("/UusiVuokrausServlet")
public class UusiVuokrausServlet extends HttpServlet {
	private static final long serialVersionUID = 1L;
		...
		protected void doGet(HttpServletRequest request,
			...
			if (action != null && action.equalsIgnoreCase("tee uusi vuokraus")) {
				...
				disp = request.getRequestDispatcher(SiteServlet.SECOND_PAGE);
				disp.forward(request, response);
			}
		...
		private void jatka(HttpServletRequest req, HttpServletResponse res)
			...
			try {
					...
					disp = req.getRequestDispatcher(SiteServlet.FRONT_PAGE);
					disp.forward(req, res);
				}
			}
				...
				if (date.compareTo(date2) <= 0) {
					try {
						...
						if (lista != null && lista2 != null) {
							...
							req.setAttribute("TK_VIRHE", true);
							disp = req
									.getRequestDispatcher(SiteServlet.FRONT_PAGE);
							disp.forward(req, res);
						}
					} catch (SQLException e) {
						req.setAttribute("TK_VIRHE", true);
		...
		private void talleta (HttpServletRequest request,
			...
			try {
				...
				disp = request.getRequestDispatcher(SiteServlet.FRONT_PAGE);
				disp.forward(request, response);
			}
